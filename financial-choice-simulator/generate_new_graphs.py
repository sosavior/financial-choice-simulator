...
# --- REPRODUCIBILITY CONSTANTS ($N = 5,000$) ---
N_SIMS = 5000
MONTHS = 12
R_STD = 0.21 / 12
R_PRED = 3.91 / 12
...
def run_simulation(global_sludge, intervention_month=None, seed=42):
    if seed is not None:
        np.random.seed(seed)
...
# --- GRAPH A: DEBT DIVERGENCE (95th Percentile) ---
print("Generating debt_divergence.png...")
...
# --- GRAPH B: CASCADE SWEEP (Cumulative Rate Eq. 13) ---
print("Generating cascade_sweep.png...")
sludge_levels = np.linspace(0.0, 1.0, 11)
cascade_rates = []

for s in sludge_levels:
    sim = run_simulation(global_sludge=s)
    # Using the formal cumulative cascade rate definition (captured in >= 1 month)
    rate = sim['capture'].any(axis=0).mean() * 100
    cascade_rates.append(rate)

fig_b = go.Figure()
fig_b.add_trace(go.Scatter(x=sludge_levels, y=cascade_rates, mode='lines+markers', line=dict(color='black', width=3)))

fig_b.update_layout(
    title='Cascade Rate as a Function of Administrative Sludge',
    xaxis_title='Administrative Sludge Coefficient',
    yaxis_title='Cascade Rate (%): Households Ever Captured',
    template='simple_white'
)
pio.write_image(fig_b, 'cascade_sweep.png', width=800, height=500, scale=2)

print("Graphs successfully generated with N=5,000!")